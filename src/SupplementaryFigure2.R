...
dev.off()
tiff('outputs/figures/SupplementaryFigure2.tiff', res=300, units='in', width=11, height=11)
...
plot.new()
text(0.5,0.55,expression(paste(italic("P. fluorescens"))), cex=1.75)
...
plot(invexp.data$sbw25.cfu.24h~divandgrowth$mG7, 
...
model<-lm(invexp.data$sbw25.cfu.24h~divandgrowth$mG7)
...
plot(invexp.data$sbw25.cfu.24h~divandgrowth$mN7,
     cex=1.5, pch=21, bg=palette_heat, 
...
model<-lm(invexp.data$sbw25.cfu.24h~divandgrowth$mN7)
...
plot(invexp.data$sbw25.cfu.24h~divandgrowth$mP7, 
     cex=1.5, pch=21, bg=palette_heat, 
...
model<-lm(invexp.data$sbw25.cfu.24h~divandgrowth$mP7)
...
plot(invexp.data$sbw25.cfu.24h~divandgrowth$mX7, 
     cex=1.5, pch=21, bg=palette_heat, 
     ylim=c(3,7),xlim=c(0.5,1.8),
...
model<-lm(invexp.data$sbw25.cfu.24h~divandgrowth$mX7)
text(1.6,6.9,substitute(paste("R"^2," = ",rsq), list(rsq=round(summary(model)$r.squared,2))), cex=2.5)
summary(model)
#Prediction intervals
pred.int =  predict(model,interval="prediction")
